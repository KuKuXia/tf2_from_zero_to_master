...
print('Original data shapes: ', x.shape, y.shape, x_val.shape, y_val.shape)

# 因为y是[50k, 1]而不是[50k]，因此使用squeeze将多余的维度删除
y = tf.squeeze(y)
...
class MyDense(layers.Layer):
    ...
    def __init__(self, inp_dim, outp_dim):
        ...
        self.bias = self.add_weight('b', [outp_dim])  # 刻意去掉bias

    def call(self, inputs, training=None):
        x = inputs @ self.kernel + self.bias
        # x = inputs @ self.kernel # 可以去掉bais做比较
        return x
...
del network
print('saved to ./model/weights.ckpt')
